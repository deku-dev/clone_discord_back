...
router.post('/server/join', async (req: Request, res: Response) => {
  ...
  if (!serverId || !userId) {
    ...
    if (await userExists(userId)) {
      const [rows] = await joinServer(serverId, userId);
      res.send(`Server ${rows[0].server_name} with ID ${serverId} Joined`);
    } else {
      res.status(401).send('Not authorized, stop that!"');
    }
  }
...
router.post('/server/rename', async (req: Request, res: Response) => {
  ...
  if (!serverName || !serverId || !userId) {
    ...
    if (await userIsAdmin(userId, serverId)) {
      await renameServer(serverName, serverId);
...
router.get('/server/activeusers', async (req: Request, res: Response) => {
  ...
  const [rows] = await getActiveUsers(serverId);
  res.send(rows);
});
...
const deleteServer = async (serverId: string) => {
  await sql.query(`DELETE FROM servers where server_id = ${sql.escape(serverId)}`);
};
...
const getActiveUsers = (serverId: string) => {
  ...
  WHERE userservers.server_id = ${sql.escape(serverId)} AND users.user_last_active > (NOW() - INTERVAL 20 MINUTE) GROUP BY users.user_id`);
};
